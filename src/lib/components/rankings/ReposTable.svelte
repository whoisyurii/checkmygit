	...
	<Table.Body>
		{#each repos as repo (repo.nameWithOwner)}
			<Table.Row href="/{repo.owner.login}">
				<Table.Cell class="font-medium text-text-secondary">
					{repo.rank}
				</Table.Cell>
				<Table.Cell>
					<div class="flex items-center gap-3">
						...
						<div class="min-w-0">
							<a
								...
								class="font-medium text-text-primary hover:text-accent-blue truncate block"
								onclick={(e) => e.stopPropagation()}
							>
								{repo.nameWithOwner}
							</a>
							{#if repo.description}
								<p class="text-sm text-text-secondary truncate max-w-[300px] md:max-w-[400px]">
									{repo.description}
								</p>
							{/if}
						</div>
